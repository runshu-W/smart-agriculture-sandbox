...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (view !== null && view !== "student" && view !== "teacher") return Response.json({ error: "无效视图" }, { status: 400 });
  try {
    const { id } = await params;
    const report = await getClassroomArchive(id, view === "teacher" ? "TEACHER" : view === "student" ? "STUDENT" : undefined);
