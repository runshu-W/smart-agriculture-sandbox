...
import { UnitThreeOperation } from "@/components/unit-three-operation";
...
export type UnitThreeCompletion = {
  summary: Record<string, boolean | number | string>;
  privateContent?: Record<string, string | string[]>;
};

export function UnitThreeActivity({ interaction, content, evidence, nextRoute }: { interaction: UnitThreeInteraction; content: ActivityContent; evidence: UnitThreeEvidence; nextRoute: string }) {
  ...
  const [retry, setRetry] = useState<UnitThreeCompletion | null>(null);

  const ensureAttempt = useCallback(async () => {
    if (attemptId.current) return attemptId.current;
    if (!starting.current) {
      starting.current = beginSimpleInteraction(interaction.id, "unit-three-map").then((attempt) => {
  ...
  async function finish(result: UnitThreeCompletion) {
    if (pending) return;
  ...
  return <main className="experience-shell full-activity-shell unit-three-activity-shell">
    <header className="experience-header"><Link aria-label="返回第三单元地图" href="/student/unit-03"><ArrowLeft /></Link><div><small>第三单元 · {interaction.section}</small><strong>{interaction.title}</strong></div><span className="step-label">{phase === "action" ? "情境操作" : "证据已保存"}</span></header>
    <section className={`unit-activity-stage unit-three-stage phase-${phase}`}>
      <Image alt="" fill priority sizes="100vw" src={content.background} /><div className="activity-stage-shade u03-stage-shade" />
      <div className="activity-workspace u03-workspace"><div className="activity-brief u03-brief"><span>{content.eyebrow}</span><h1>{interaction.title}</h1><p>{content.description}</p><small>{content.instruction}</small><div className="psych-safety"><HeartHandshake /><p><b>安全与隐私</b>人际风格没有人格优劣；开放文字只保存到个人私密内容。</p></div></div><ManagedQuestionsProvider interactionId={interaction.id} onAnswer={recordQuestionResponse}><UnitThreeOperation evidence={evidence} interaction={interaction} onComplete={finish} pending={pending} /></ManagedQuestionsProvider></div>
      {phase === "complete" && <InteractionCompletionDialog description="情境操作、策略分支、完成状态和用时已保存；私密原文不会进入教师看板。" mapRoute="/student/unit-03" nextRoute={nextRoute} title={interaction.title} />}
