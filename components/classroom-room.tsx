...
export function ClassroomRoom({ initial, view }: { initial: LessonSnapshot; view: View }) {
  const canControl = view === "teacher" || view === "screen";
  ...
  async function send(command: LessonCommand | { action: "join" | "ready" }, version = data.version) {
    ...
    try {
      const response = await fetch(`/api/classroom/${data.id}`, { method: canControl ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...command, version }), signal: AbortSignal.timeout(10_000) });
...
{view === "student" && data.status !== "ENDED" && <div className="classroom-student-actions">{!data.ownJoined ? <button className="classroom-primary" disabled={disabled} onClick={() => void send({ action: "join" })}><UsersRound size={18} />加入本次课堂</button> : <button className={`classroom-primary ${data.ownReady ? "ready" : ""}`} disabled={disabled || data.ownReady} onClick={() => void send({ action: "ready" })}><CheckCircle2 size={18} />{data.ownReady ? "已准备好，等待老师安排" : "我准备好了"}</button>}<span>{data.ownJoined ? "你已加入课堂，刷新后会恢复当前进度。" : "加入后，老师就能看到你的到场状态。"}</span></div>}
    <div className="classroom-stage-track" aria-label="课堂六阶段">{CLASSROOM_STAGES.map((item, index) => {
      ...
      return <button key={item.key} className={`${current ? "current" : ""} ${index < data.stage ? "past" : ""}`} disabled={!canControl || disabled || data.status === "WAITING" || current} onClick={() => confirmStage(index)} aria-current={current ? "step" : undefined}><span>{index < data.stage ? <Check size={15} /> : `0${index + 1}`}</span><b>{item.title}</b>{!canControl && index > data.stage && <LockKeyhole size={13} />}</button>;
    })}</div>
    <div className="classroom-body">
      ...
      {canControl ? <aside className="classroom-control-panel"><div className="classroom-panel-heading"><h2>授课节奏</h2><span>{data.className}</span></div><p>老师控制阶段，学生端自动跟随。</p><div className="classroom-controls">
    ...
    {view !== "screen" && (view === "teacher" || data.ownJoined) && <section className="classroom-export-bar"><div><h3>本次课堂档案</h3><p>{view === "student" ? "回看自己的变化地图与学习过程，可下载留存。" : "导出班级汇总与变化地图，个人决策和答题明细仅学生本人可见。"}</p></div><a className="classroom-secondary" href={`/api/classroom/${data.id}/archive?view=${view}&format=html`} target="_blank" rel="noopener noreferrer">预览 / 打印档案</a><a className="classroom-secondary" href={`/api/classroom/${data.id}/archive?view=${view}&format=json`}>下载课堂记录</a>{view === "student" && <Link href="/student/progress" className="classroom-secondary">我的成长档案</Link>}</section>}
